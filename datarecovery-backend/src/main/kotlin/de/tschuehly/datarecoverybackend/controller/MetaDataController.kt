package de.tschuehly.datarecoverybackend.controller

import de.tschuehly.datarecoverybackend.helpers.CrudController
import de.tschuehly.datarecoverybackend.model.Customer
import de.tschuehly.datarecoverybackend.model.MetaData
import de.tschuehly.datarecoverybackend.repository.CustomerRepository
import de.tschuehly.datarecoverybackend.repository.MetaDataRepository
import de.tschuehly.datarecoverybackend.service.CustomerService
import de.tschuehly.datarecoverybackend.service.MetaDataService
import org.springframework.web.bind.annotation.RequestMapping
import org.springframework.web.bind.annotation.RestController

@RestController
@RequestMapping("api/metadata")
class MetaDataController(metaDataService: MetaDataService):
    CrudController<MetaData,MetaDataRepository,MetaDataService>(metaDataService){
}
