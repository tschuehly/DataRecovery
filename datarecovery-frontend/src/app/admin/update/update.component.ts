...
@Component({
  ...
  template: `
    <div class="flex flex-col">
      ...
      <label
        *ngFor="let question of this.order.orderProduct.category.questions"
        class="text-center my-4"
      >
        {{ question }}
        <input
          type="radio"
          (click)="addQuestion(question, 'Ja')"
          name="question"
        />
        Ja
        <input
          type="radio"
          (click)="addQuestion(question, 'Nein')"
          name="question"
        />
        Nein
      </label>
      ...
      >
        <textarea
          class="block mt-2 w-full h-96 m-auto"
          formControlName="description"
        ></textarea>
        <div
          class="flex flex-col my-4"
          *ngFor="let picture of pictureDetails.controls; let i = index"
        >
          <label
            >Bildtitel
    ...
    for (let i = 0; i < this.files.length; i++) {
      ...
      if (this.pictureDetails.controls[i].value === '') {
        ...
        updateData.append(
          'pictures',
          this.files[i],
          this.pictureDetails.controls[i].value
        );
      }
    }
    updateData.append(
      'update',
      JSON.stringify(this.updateForm.getRawValue() as Update)
    );
    this.http
      .post('api/order/addUpdate/' + this.order.id, updateData)
