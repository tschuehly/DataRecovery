      ...
      <div class="m-auto" *ngIf="!editProduct">
        ...
        <table class="border table-auto mx-auto">
          ...
          <tbody>
            <tr *ngFor="let product of products">
              ...
              <td class="border p-2">
                {{ product?.category?.replacement ? 'Ja' : 'Nein' }}
              </td>
              ...
              <ng-container *ngIf="showDelConfirm">
                <td class="border pl-2">
                  <button
                    class="border p-2 bg-red-400 m-4"
        ...
        <div class="text-right mt-4">
          <button
            ...
            (click)="editProduct = emptyProduct()"
          >
            Neues Produkt
          </button>
        </div>
...
export class ProductComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get('api/product').subscribe((products: Product[]) => {
      this.products = products
        .sort((p1, p2) => p1.category?.name?.localeCompare(p2?.category?.name))
  ...
  saveProduct(productToSave: Product): void {
    this.http
      .post('api/product', productToSave)
