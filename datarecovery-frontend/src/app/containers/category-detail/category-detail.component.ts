  ...
  template: `
    <div>
      ...
      <div class="grid grid-cols-1 text-lg gap-4">
        <form [formGroup]="editCategoryForm" class="mx-auto text-center grid grid-cols-2 gap-2 justify-items-center">
          <label>Name:
          ...
          <label>Neue Frage
            ...
            <button (click)="addQuestion(question.value)"  class=" my-4 border-2 rounded-md p-2 border-black">Neue Frage hinzufügen</button>
          </label>
        </form>
        <div class="flex justify-between mt-6">
          <button (click)="saveCategory()" class="border-2 rounded-md p-2 border-black">Speichern</button>
          <button (click)="showConfirm = true" class="border-2 rounded-md p-2 bg-red-500 border-black">Kategorie löschen</button>
          <button (click)="close.emit()" class="border-2 rounded-md p-2 border-black">Schließen</button>
        </div>
        <div class="grid grid-cols-1 gap-4 text-center justify-center mt-6" *ngIf="showConfirm">
          Bist du dir sicher das du die Kategorie löschen willst?
          <button (click)="delete.emit(category)" class="border-2 rounded-md p-2 bg-red-500 border-black">Ja lösche die Kategorie</button>
        </div>
      </div>
    </div>
  `,
...
export class CategoryDetailComponent implements OnInit {
  ...
  addQuestion(value: string): void {
    ...
    (this.editCategoryForm.get('questions') as FormArray).push(this.fb.control(value));
  }
